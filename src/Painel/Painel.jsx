...
const Painel = () => {
  ...
  const fetchUserData = useCallback(async () => {
    try {
      ...
      const response = await fetch(`http://localhost:30120/user?email=${encodeURIComponent(email)}`, {
        headers: {
  ...
  const fetchDisciplinasEmAndamento = useCallback(async () => {
    try {
      const token = sessionStorage.getItem('token');
      const response = await fetch(`http://localhost:30120/logs?email=${encodeURIComponent(email)}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleIniciarAula = async () => {
    ...
    try {
      const response = await fetch('http://localhost:30120/logs', {
        method: 'POST',
        headers: {
